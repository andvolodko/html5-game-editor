import { isSpineImportFile, isSupportedTextureFile } from "@game-editor/assets";
...
export async function importDroppedFiles(
  ...
  const supported = files.filter(
    (file) => isSupportedTextureFile(file) || isSpineImportFile(file),
  );
  if (supported.length === 0) {
    return {
      importedCount: 0,
      errors: [],
      message: "No supported files (png/jpg/webp or Spine json/skel/atlas)",
    };
  }
